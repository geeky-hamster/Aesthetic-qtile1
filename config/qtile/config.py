...
def autostart():
    ...
    subprocess.Popen([home + '/.config/scripts/autostart'])
...
home         = os.path.expanduser('~')
terminal     = home + '/.config/scripts/fl_term'
music_player = 'ncspot'
color_picker = home + '/.config/scripts/colorpick'
brightness   = home + '/.config/scripts/brightness'
volume       = home + '/.config/scripts/volume'
...
rofi_applets = home + '/.config/rofi/applets/bin/'
...
keys = [
    ...
    Key(
		["mod1"], "F1", 
		lazy.spawn(".config/rofi/launchers/type-4/launcher.sh"), 
		desc="Run application launcher"
	),
    Key(
		[mod], "n", 
		lazy.spawn(rofi_applets + 'brightness.sh'), 
		desc="Run command runner"
	),
    Key(
		[mod], "x", 
		lazy.spawn(rofi_applets + 'powermenu.sh'), 
		desc="Run powermenu applet"
	),
    Key(
		[mod], "m", 
		lazy.spawn(rofi_applets + 'mpd.sh'), 
		desc="Run music player applet"
	),
    Key(
		[mod], "r", 
		lazy.spawn(rofi_applets + "appasroot.sh"), 
		desc="Run asroot applet"
	),
    Key(
		[mod], "s", 
		lazy.spawn(rofi_applets + 'screenshot.sh'), 
		desc="Run screenshot applet"
	),
    Key(
		[mod], "l", 
		lazy.spawn(rofi_applets + 'quicklinks.sh'), 
		desc="Run screenshot applet"
	),
    Key(
		[mod], "t", 
		lazy.spawn(rofi_applets + 'battery.sh'), 
		desc="Run screenshot applet"
	),
    Key(
		[mod], "z", 
		lazy.spawn('/home/soham/.config/scripts/rofi_runner'), 
		desc="Run screenshot applet"
	),    
    Key(
		[mod], "w", 
		lazy.spawn('/home/soham/.config/scripts/rofi_windows'), 
		desc="Run screenshot applet"
	),  
...
groups = [Group("1", layout='MonadTall'),
          Group("2", layout='MonadTall', matches=[Match(wm_class=["brave-browser"])]),
          Group("3", layout='MonadTall', matches=[Match(wm_class=["thunar"])]),
          Group("4", layout='MonadTall', matches=[Match(wm_class=["subl"])]),
          Group("5", layout='MonadTall', matches=[Match(wm_class=["code"])]),
          Group("6", layout='MonadTall'),
          Group("7", layout='MonadTall'),
          Group("8", layout='MonadTall', matches=[Match(wm_class=["lxappearance"])]),
          Group("9", layout='MonadTall', matches=[Match(wm_class=["ncspot, Spotify"])]),
...
for i in groups:
    ...
    keys.extend([
        ...
        Key([mod], group_letter,
            lazy.group[group_name].toscreen(),
            desc="Switch to group {}".format(group_name),
        ),

        Key([mod, "shift"], group_letter,
            lazy.window.togroup(group_name, switch_group=True),
            desc="Move focused window & switch to group {}".format(group_name),
        ),

        Key([mod, "control"], group_letter,
    ...
    # Now do same key action applied to the Numpad keys.
    ...
    keys.extend([
        ...
        Key([numlock, mod], 'KP_' + KP[group_letter],
            lazy.group[group_name].toscreen(),
            desc="Switch to group {}".format(group_name)
        ),

        Key([numlock, 'shift'], 'KP_' + KP[group_letter],
            lazy.window.togroup(group_name, switch_group=True),
            desc="Move focused window & switch to group {}".format(group_name),
        ),

        Key([numlock, 'control'], 'KP_' + KP[group_letter],
            lazy.window.togroup(group_name, switch_group=False),
            desc="Move focused window to group {}".format(group_name),
        ),
...
floating_layout = layout.Floating(
    ...
    float_rules=[
        ...
        *layout.Floating.default_float_rules,
        ...
        Match(wm_class="feh|Viewnior|Gpicview|Gimp|MPlayer|Vlc"),
